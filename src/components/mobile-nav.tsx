...
import { Menu, X, LogOut, School, ChevronRight } from "lucide-react";
import { cn } from "@/lib/utils";
import { ROLE_LABELS } from "@/lib/rbac";
import { APP_NAV, getPageTitle } from "@/components/app-nav";
...
export function MobileNav() {
  ...
  return (
    <div className="md:hidden print:hidden">
      <div className="sticky top-0 z-40 border-b border-border/70 bg-card/95 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-card/85">
        <div className="flex items-center justify-between gap-3 px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-top))]">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <School className="size-5 shrink-0 text-primary" />
              <div className="truncate text-base font-bold text-foreground">PRS SPBMS</div>
            </div>
          ...
          <button
            ...
            className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-border/80 bg-background text-foreground shadow-sm transition-colors hover:bg-muted"
      ...
      >
        <div className="border-b border-border/70 bg-muted/20 px-4 py-4 pb-[max(1rem,env(safe-area-inset-top))]">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <School className="size-6 shrink-0 text-primary" />
                <div>
                  <div className="font-bold">PRS SPBMS</div>
                  <div className="text-xs text-muted-foreground">แผนปฏิบัติการ & งบประมาณ</div>
                </div>
              </div>
              <div className="mt-4 rounded-2xl border border-border/70 bg-background/80 px-3 py-2">
            ...
            <button
              onClick={() => setIsOpen(false)}
              className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-border/80 bg-background text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
              ...
              <Link
                ...
                className={cn(
                  ...
                  active
                    ...
                    : "border-transparent bg-background hover:border-border/80 hover:bg-muted/70",
                )}
              >
                <span
                  className={cn(
        ...
        <div className="border-t border-border/70 bg-muted/10 px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
          <div className="mb-4 rounded-2xl border border-border/70 bg-background/90 px-4 py-3 shadow-sm">
