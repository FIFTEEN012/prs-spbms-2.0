...
export function CreatePurchaseRequestForm({
  ...
  const [subject, setSubject] = useState(`ขออนุมัติซื้อวัสดุ/ครุภัณฑ์เพื่อดำเนินโครงการ ${project.projectName}`);
  const [activityId, setActivityId] = useState("");
  const [category, setCategory] = useState("SUPPLIES");
  const [categoryDetails, setCategoryDetails] = useState("");
  const [fundSourceId, setFundSourceId] = useState(project.fundSourceId || "");
  const [budgetWalletId, setBudgetWalletId] = useState("");
  const [actionPlanPage, setActionPlanPage] = useState("");
  const [isNotInPlan, setIsNotInPlan] = useState(false);
  ...
  const handleSubmit = async (
    ...
    // For no-activity case: compare against project budget
    ...
    try {
      ...
      setFormSuccess("บันทึกคำขอจัดซื้อจัดจ้างเรียบร้อยแล้ว");
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="md:col-span-3 space-y-4 p-4 border rounded-lg bg-amber-50 border-amber-300 dark:bg-amber-950/20 dark:border-amber-900/50 text-amber-900 dark:text-amber-200">
                ...
                {otherProjects.length > 0 && (
                  <div className="pt-2 border-t border-amber-200 dark:border-amber-900 space-y-3">
                    ...
                    {/* Show selected cross-project items summary */}
      ...
      <div className="flex gap-4 justify-end">
        ...
        <Button type="submit" disabled={loading}>
          {loading ? "กำลังบันทึก..." : "ส่งคำขออนุมัติจัดซื้อจัดจ้าง"}
        </Button>
      </div>
    </form>
  );
